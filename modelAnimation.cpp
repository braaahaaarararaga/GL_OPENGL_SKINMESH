...
void InitModelAnimation()
{
	...
	g_Scene = aiImportFile(modelPath.c_str(), aiProcessPreset_TargetRealtime_MaxQuality);
...
void DrawModelAnimation()
{
	...
	glPushMatrix();
	// matrix setting
	DrawModelAnimationMesh(g_Scene->mRootNode);
	glPopMatrix();
}
...
void DrawModelAnimationMesh(const aiNode * Node)
{
	...
	{
		DrawModelAnimationMesh(Node->mChildren[n])
			;
	}
	glPopMatrix();
}
